...
def get_doc_templates() -> JinjaTemplates:
    ...
    templates_folder = f"{current_path}\\docs_templates\\"
    act_template = get_act_doc_template(templates_folder)
    task_template = get_task_doc_template(templates_folder)

    return [act_template, task_template]
...
def generate_report_templates() -> None :
    ...
    current_path = os.path.dirname(os.path.realpath(__file__))
    for info in infos:
        formatted_tex_file_path = format_tex_file_by_doc_template(info, current_path)
